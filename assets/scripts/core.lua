-- Core lua only objects like vectors, colors, etc
...
function Vec2Args(a, b)
    if type(a)=="number" then return Vec2(a),b end
    if type(b)=="number" then return a,Vec2(b) end
    return a,b
end
Vec2_meta = {
    __unm = function(tble)
...
function Vec2(x, y)
    if getmetatable(x)==Vec2_meta then -- Copy
        local v = {
            x = x.x,
            y = x.y
        }
...
function Color(r, g, b, a)
    if getmetatable(r)==Color_meta then -- Copy
        local v = {
            r = r.r,
            g = r.g,
            b = r.b,
